...
# print("Anz : Landkreis")
for id, value in sorted(d_counter_region.items(), key=lambda item: item[1], reverse=True):
    if value < 5:
        break
    lk_name = d_districts_latest[id]['Landkreis'].encode(
        'ascii', 'xmlcharrefreplace').decode()
    # lk_name = lk_name.replace("ö", "&ouml;")
    # print(f"%3d : {d_districts_latest[id]['Landkreis']}" % value)
    print(
        f"<tr><td>{value}</td><td>{lk_name}</td></tr>"
    )
print("</table>")
